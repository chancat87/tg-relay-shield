...
* 🔕 **防刷屏智能等待回执（10 分钟静默窗口）**：
  * 彻底解决连续发信时每条都提示“请耐心等待回复”的刷屏痛点。单会话周期内实施 10 分钟静默窗口，仅在初次或超过 10 分钟后的首条消息提示一次，窗口期内连续发信静默转达，聊天体验清爽自然。
* ⚡ **3 次答错熔断锁死 (Anti-DDoS) 与 60 秒重置冷却**：
  * 连续答错 3 次自动熔断锁定 30 分钟，锁定期间对恶意连点实施零写 KV 静默拦截，保卫免费额度。
  * 生产环境已验证用户自动收敛常驻重验按键，`/reset` 重置指令与历史回调施加 60 秒冷却防刷，从根源粉碎自动化脚本刷题攻击面。
* 🌐 **纯正双语隔离与自适应一键切换**：
...
* 🛠️ **身份隔离菜单体系 (Scope-based)**：
  * 自动为普通访客（极简 `/start`、`/reset`、`/about`，最后一行清晰标注为 **“关于”**）与管理员（专属 `/block`、`/unblock`、`/addkw`、`/delkw`、`/listkw`、`/help` 等）配置物理隔离的操作指令菜单。
