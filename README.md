...
* 🛡️ **极简“双模防御”热切换 (`/kqfy`)**：
  * **模式 1 (标准模式)**：默认 Emoji 趣味计数验证，兼顾高体验与高防刷，**绝不误伤任何正常文章、博客链接交流**。
  * **模式 2 (终极模式)**：Cloudflare 官方网页人机盾牌 (Turnstile / Web Challenge)。遭遇脚本轰炸时一键开启，降维打击，杜绝暴力破解。
  * 支持带参指令 `/kqfy 1`、`/kqfy 2` 秒切，或直接输入 `/kqfy` 弹出可视化点选按键面板。
