...
* 🍓 **趣味 Emoji 动态视觉算术（标准模式）**：
  * 拒绝死板的固定题库与纯文本数字！采用 16 种无歧义通用 Emoji 动态生成（如 `🍎🍎 + 🍎🍎🍎 = ?`）。
  * 题目文本**完全不含任何阿拉伯数字**，彻底废掉传统正则爬虫；按键选项则采用**纯标准数字**（如 `[ 5 ]`），人类 0.5 秒心算秒点，绝无需眯眼数按键。
* 🛡️ **极简“双模防御”热切换 (`/kqfy`)**：
  * **模式 1 (标准模式)**：默认 Emoji 趣味计数验证，兼顾极速体验与防刷，**绝不误伤任何正常文章、博客链接交流**。
  * **模式 2 (终极模式)**：Cloudflare 官方网页人机盾牌 (Turnstile / Web Challenge)。通过 **Telegram Mini App (小程序)** 原生唤起，零外部弹窗，采用加密随机一次性票据彻底隐匿 UID；免配置默认启用 Cloudflare 官方交互式验证盾，支持无缝绑定自定义生产密钥。
  * 支持带参指令 `/kqfy 1`、`/kqfy 2` 秒切，或直接输入 `/kqfy` 弹出可视化按键面板。
* 💬 **访客引用回复 (Quote) 上下文置顶还原**：
  * 完美解决 Telegram 转发机制丢失引用气泡的痛点。当访客引用历史图片、贴纸、文字或文件提问时，管理员私聊会在转发上方自动置顶推送原图文摘要。
* ⚡ **3 次答错熔断锁死 (Anti-DDoS)**：
...
* 🛠️ **身份隔离菜单体系 (Scope-based)**：
  * 自动为普通访客（极简 `/start`、`/about`）与管理员（专属 `/kqfy`、`/block`、`/addkw` 等）配置物理隔离的操作指令菜单。
...
| **`/kqfy`** | `/kqfy` 或 `/kqfy 1\|2` | **【双模防御热切换】**<br>直接输入 `/kqfy 1` (标准Emoji) 或 `/kqfy 2` (终极网页盾)，或发送 `/kqfy` 弹出按键面板 |
